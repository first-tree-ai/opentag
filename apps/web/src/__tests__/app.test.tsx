      ...
      return json(
        {
          ...
          agentId,
          intent: body.intent,
          // The Server reports the brand it actually minted against, defaulting where none was named.
...
describe("OpenTag Web App Shell", () => {
  ...
  it("creates a Feishu setup attempt only after an explicit admin click", async () => {
    ...
    /*
     * The code names the brand it was minted against, which a first connect decides from the
     * reader's language before anything is authorized — so the assertion follows the request rather
     * than assuming Feishu. The way back to the other brand stands beside it, because a language is
     * a guess about a company rather than a fact about it.
     */
    const mintRequest = vi
      .mocked(fetch)
      .mock.calls.find(
        ([input, init]) => String(input).endsWith("/im-binding/feishu/setup-attempts") && init?.method === "POST",
      );
    const minted = (JSON.parse(String(mintRequest?.[1]?.body)) as { brand?: "feishu" | "lark" }).brand ?? "feishu";
    ...
    expect(within(dialog).getByRole("button", { name: `Use ${otherLabel} instead` })).toBeTruthy();
    await waitFor(() =>
      expect(
        vi
          .mocked(fetch)
