 ...
 *
 * The fixed-row primitive renders four lines regardless of stage, so every preparation stage keeps
 * the same four rows and the same DOM identity while the copy underneath resolves.
 */
 ...
 * computer-preparation stage, and it remains visible as the successful summary while Messaging is
 * still not-configured — so all four ready rows are on screen before the first IM interaction.
 * Once Messaging has started (authorizing, waiting-handoff, blocked, observation-failed, ready)
 * the rows leave the page: an unselected Provider's later report must never regress a messaging
 * session that is already underway.
 */
export function showPreparationSection(snapshot: AgentSetupSnapshot): boolean {
  if (snapshot.stage === "needs-messaging") return snapshot.messaging.kind === "not-configured";
  return (
