...
import { imConversations } from "./integrations.js";
...
export const sessions = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("sessions_active_thread_unique")
      .on(table.conversationId, table.threadKey)
      .where(sql`${table.kind} = 'thread' and ${table.endedAt} is null`),
    index("sessions_conversation_id_idx").on(table.conversationId),
    check(
...
export const sessionsRelations = relations(sessions, ({ one }) => ({
  conversation: one(imConversations, { fields: [sessions.conversationId], references: [imConversations.id] }),
