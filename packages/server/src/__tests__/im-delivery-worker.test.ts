...
describe("ImDeliveryWorker database workflow", () => {
  ...
  it("does not expire pending deliveries through the janitor while paused", async () => {
    const fixture = await workerFixture(unit);
    await unit.database
    ...
    try {
      ...
      const [delivery] = await unit.database
        .select({ state: imMessageDeliveries.state, reason: imMessageDeliveries.reason })
        .from(imMessageDeliveries)
        .where(eq(imMessageDeliveries.id, fixture.deliveryId));
      expect(delivery).toEqual({ state: "pending", reason: null });
    } finally {
      worker.stop();
    }
  });

  it("keeps the delivery scheduler usable after the ownership fence resumes", async () => {
    const fixture = await workerFixture(unit);
    const custody = new PostgresRuntimeCustodyStore(unit.database);
    ...
    try {
      ...
      expect(domain.requestDelivery).toHaveBeenCalledOnce();
      expect(await custody.getDelivery(fixture.deliveryId)).toMatchObject({ turnId: "turn-1" });
    } finally {
      worker.stop();
      runOnce.mockRestore();
  ...
  it("pauses and resumes delivery, expiry, and retention timers together", async () => {
    vi.useFakeTimers();
    const expiryExecute = vi.fn().mockResolvedValue([]);
    const retentionTransaction = { execute: vi.fn().mockResolvedValue([]) };
    const database = {
    ...
    const runOnce = vi.spyOn(worker, "runOnce").mockResolvedValue(undefined);
    ...
    try {
      ...
      expect(expiryExecute).toHaveBeenCalledTimes(4);
      expect(retentionTransaction.execute).toHaveBeenCalledTimes(8);

      worker.pause();
      worker.pause();
      expect(vi.getTimerCount()).toBe(0);
      runOnce.mockClear();
      expiryExecute.mockClear();
      retentionTransaction.execute.mockClear();
      await vi.advanceTimersByTimeAsync(80);
      expect(runOnce).not.toHaveBeenCalled();
      expect(expiryExecute).not.toHaveBeenCalled();
      expect(retentionTransaction.execute).not.toHaveBeenCalled();
