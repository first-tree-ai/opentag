    ...
    const timer = window.setInterval(() => {
      ...
      void browserApi.computers().then(
        (value) => {
          ...
          /*
           * An arrival is not evidence that this machine redeemed this code. `findArrival` accepts
           * any Computer on the Account that enrolled -- or merely reconnected -- since the
           * baseline, and takes the first of them. That is enough to name a machine on this step
           * and nowhere near enough to write one into an Agent's durable placement, which is why
           * this used to rebind an Agent that had no Computer and no longer does: a wrong guess
           * would have run the Agent somewhere its owner never chose, permanently. Issue #295
           * carries the connect-code-to-Computer association that would make the move provable;
           * until it exists there is no subject here to move an Agent onto.
           */
