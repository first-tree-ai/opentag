...
Task 详情列出话题内消息的投递作为执行记录。私聊按时间正序展示，每轮先显示入站消息、再显示 Agent 回复，
最新轮次位于底部。**加载更早活动**位于对话上方，将更早记录插入顶部，并保持当前可见轮次的阅读位置。
群聊与频道话题仍按最新执行在前展示。所有会话类型的 API 都先获取最新一页，Web 将私聊的完整已加载记录
转成正序。Task 列表排序不变。
