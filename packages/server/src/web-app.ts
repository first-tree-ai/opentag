...
export function registerWebApp(app: FastifyInstance, root: string): void {
  ...
  app.addHook("onSend", async (request, reply) => {
    ...
    reply.header(
      ...
      "default-src 'self'; connect-src 'self'; img-src 'self' data: https://platform.slack-edge.com; style-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'",
    );
