...
与镜像 CLI 版本一致的 OPENTAG_CLOUD_RUNNER_VERSION，以及下表中的完整 Runner 参数。关闭总开关
会同时关闭身份、Runner 和模型能力，不受模型开关的遗留值影响。若只需暂停模型请求并保留保存／
释放能力，仅关闭 OPENTAG_CLOUD_MODEL_ENABLED。没有独立的 Runner 开关或前端可见性开关。
旧 OPENTAG_CLOUD_RUNNER_ENABLED 已不再读取；升级 Server 后可从部署配置移除，仅在需要回滚
到旧版 Server 的窗口内保留。
...
普通兼容的 Runner 升级、回滚只改变新建 Instance 的目标。此前已验证并就绪的 Instance 可用原有不可变镜像重连，
继续完成工作、保存并空闲回收；与当前目标镜像不同时不能跨 Session 借用。首次准入仍严格校验。
统一 CLI 发布与启用流程见 [Cloud Runner 发布](./cloud-runner-release.md)。

Server 跨不兼容协议回滚仍需明确顺序。E8 Runner 在 auth 中发送 `sessionCollaborationVersion`，
E8 之前 Server 的严格 schema 会拒绝该字段。降级 Server 前，应停止新工作，在兼容 Server 上完成结果确认和保存，
确认相关 Instance 已删除，再选择旧 Server 和兼容 Runner 目标。
旧 Runner 不能消费 E8 执行日志；替换 Instance 不会恢复尚未确认的收据和结算，不能盲目重放。
Session 已确认保存的工作目录在下次分配时正常恢复；协议兼容并不保证任意持久化数据降级都兼容。
