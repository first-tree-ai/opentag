...
describe("Tasks view", () => {
  ...
  it("names the channel of a Task's source instead of printing its provider id", async () => {
    ...
    /*
     * The redesigned detail page replaced SourceIdentity with this row, so the earlier
     * `Atlas · Feishu` assertion no longer has a subject: the Agent name and the channel are now
     * separate flex children, and the run-together reading that separator fixed cannot occur here.
     * What still has to hold is the naming itself — the channel reads as the product name, and the
     * lowercase identifier never reaches the reader. `not.toMatch` is case-sensitive, so it pins the
     * raw id specifically rather than merely asserting a substring.
     */
    const details = await screen.findByRole("region", { name: "Task details" });
    expect(details.textContent).toContain("Feishu");
    expect(details.textContent).not.toMatch(/feishu/);
  });
});
