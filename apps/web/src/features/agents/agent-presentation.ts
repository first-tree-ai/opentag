 ...
 * excluded: a connected Slack or Lark App can coexist with an offline Computer or unavailable
...
export function sharedConversationDestination(provider: ImBindingSummary["provider"], plural = false): string {
  if (provider === "feishu") return plural ? "connected Lark group chats" : "a Lark group chat";
  return plural ? "connected Slack channels" : "a Slack channel";
}

/**
 * Lark gives each Agent its own bot, so the handle addresses it. Slack routes one workspace Bot,
...
export function agentRecoveryMessage(agent: AgentDetailView): string {
  const messages: Record<NonNullable<AgentAvailability["reason"]>, string> = {
    ...
    im_not_connected: "Connect Lark or Slack so teammates can send this Agent work.",
    ...
    im_error: "The messaging connection failed. Reconnect Lark or Slack to receive messages.",
    im_disabled: "Messaging is turned off for this Agent. Reconnect Lark or Slack to receive messages.",
