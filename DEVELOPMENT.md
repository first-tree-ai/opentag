...
Run these commands from the root of your cloned repository. This walkthrough uses loopback-only developer sign-in; see the sections below for additional configuration.
...
| CLI | Commander; `opentag-dev` from a checkout, `opentag` for the production channel |
...
The README covers the hosted service. Use the local walkthrough above to run from a checkout; the sections below
cover development checks, individual setup steps, and advanced configuration.
