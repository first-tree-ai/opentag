...
it("checkpoints a completed Turn, seals after report ACK, and cold-restores Pi state over real sockets", async () => {
  ...
  try {
    ...
    expect(await readFile(join(second.workspace, ".opentag/pi-session/binding.json"), "utf8")).toContain("pi-original");
