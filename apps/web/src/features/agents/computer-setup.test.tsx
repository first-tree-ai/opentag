...
describe("ComputerSetup", () => {
  ...
  it("keeps waiting while the verdict is pending, whatever the Computers list shows", async () => {
    const onConnected = vi.fn();
    // A new Computer registering and an existing one reconnecting are both just list movement: no
    // verdict, no arrival.
  ...
  it.each(["expired", "revoked"] as const)(
    ...
    async (state) => {
      ...
      // that happens to be connected nearby is not adopted.
...
describe("ComputerList", () => {
  it("explains when no Computers are connected", () => {
    render(<ComputerList computers={[]} />);

    expect(screen.getByRole("heading", { name: "Connected Computers" })).toBeTruthy();
    expect(screen.getByText("No Computers are connected yet.")).toBeTruthy();
  });
});
