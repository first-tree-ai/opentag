...
## Skills change notification

The optional `runtime.skillsSync` capability (version 1) lets the Server tell a connected Client when the skill set assigned to one of its agents changed. When negotiated, the Server may send the business frame `skills:changed` with `agents: [{ agentId, digest }]` after a skill upload, replacement, deletion, or assignment change. The Client syncs only the agents it has already prepared a workspace for, compares the advertised agent digest with its local `.skills/.opentag-skills.json`, and fetches the runtime skills manifest and any changed archives over HTTP with its machine token. The frame is a hint, not a transaction: the Client also verifies its local skill trees on workspace preparation, on each Turn admission, and on a periodic sweep, and `EffectiveRuntimeSnapshot.skills.digest` carries the same agent digest so a stale local tree is noticed even when a frame is missed. Clients that did not negotiate the capability never receive the frame.
