...
describe("An Agent with no Computer", () => {
  ...
  it("gives the Agent the Computer this Account has, without asking which", async () => {
    ...
    await renderInRouter(<AgentComputerChoice agentId={UNBOUND_AGENT_ID} onBound={onAgentChanged} />);
  ...
  it("says the Computer read failed instead of reporting an Account with no Computer", async () => {
    ...
    await renderInRouter(<AgentComputerChoice agentId={UNBOUND_AGENT_ID} onBound={() => undefined} />);

    expect(await screen.findByRole("button", { name: "Try again" })).toBeTruthy();
    expect(screen.getByText(/Couldn’t load your computers/)).toBeTruthy();
    expect(screen.queryByText("Connect computer")).toBeNull();
    expect(screen.queryByRole("button", { name: /Use / })).toBeNull();
  });
  ...
  it("asks which Computer when the Account has several, and binds exactly the one chosen", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Choose a computer for this Agent" })).toBeTruthy();
    ...
    fireEvent.click(screen.getByRole("button", { name: "Use this computer: Spare" }));
  ...
  it("does not bind by itself while several Computers are connected", async () => {
    ...
    expect(await screen.findByRole("button", { name: "Use this computer: Ada's Mac" })).toBeTruthy();
    expect(rebind).not.toHaveBeenCalled();
  });
  ...
  it("binds a second unbound Agent when the same surface is reused for one", async () => {
    ...
    const { rerender } = await renderInRouter(
      <AgentComputerChoice agentId={UNBOUND_AGENT_ID} onBound={() => undefined} />,
    );
    await waitFor(() => expect(rebind).toHaveBeenCalledWith(UNBOUND_AGENT_ID, COMPUTER_ID));

    rerender(<AgentComputerChoice agentId={SECOND_AGENT_ID} onBound={() => undefined} />);
  ...
  it("keeps a newly connected Computer's bind failure visible and retryable", async () => {
     ...
     * arrived, which used to hide the failure entirely and leave the reader on "Connect computer"
