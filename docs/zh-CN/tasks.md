...
Task 详情将话题内消息与已记录的 Agent 回复合并为按时间正序排列的对话，适用于私聊、群聊和频道话题。
用户补充要求会按时间出现在回复之间。回复缺少有效时间时，保留其与相邻回复的位置关系；全部缺少时间时
放在报告时间处，但不在界面虚构发送时间。已加载页面中重复的投递和回复仅显示一次，内容相同但身份不同的
消息以及已存储的消息修订仍各自保留。

**加载更早活动**位于对话上方。API 仍先获取最新一页，Web 合并所有已加载页面后排列消息；加载历史或
收到较晚到达的报告时，保持当前可见消息的位置。Task 列表排序不变。

两类投递被排除：thread Session 已拥有的消息在 channel Session 里的 `ambient` 旁听副本，以及因消息
出现更新修订而过期的投递。并入进行中 Turn 的消息仅在用户消息旁轻量标注，不生成第二条 Agent 回复或报告。

执行摘要与已发送回复分开展示。有已记录回复时，摘要默认折叠，展开状态在后台刷新和分页后保留；没有回复时，
摘要直接显示并明确标注。执行失败、回复记录缺失或不完整、内容截断等提示不会随摘要折叠而隐藏。

入站附件展示已存储的名称、类型及处理可用性限制。Task API 从现有消息资源投影元数据，不返回提供方资源密钥。
历史消息或旧服务器没有附件元数据时仍能正常展示。此次不增加附件预览或下载接口、引用消息展开或原对话入口。
...
返回 `409`——worker 在此期间认领、接受或并入 Turn 的行属于正在进行的 Turn，而不是离开了队列，排在它后面的消息
原地保留。取消等待锁期间被 worker 拒绝或已过期的行保持 worker 写入的状态，旁边仍待处理的行照常被撤回；若已
没有任何待处理行，`409` 会说明 Task 当前实际的状态（`The Task is failed, not queued`）。Web 收到这个答复时刷新
Task 而不是报告失败，并根据刷新结果区分两种情况：已经离开队列的 Task 提示"已不在排队中"；仍为 `queued` 的
Task 提示暂时无法取消——其消息正在投递或等待确认——并保留取消按钮。这条提示覆盖排队消息的两种状态：worker
持有它的有效认领（正在投递，Computer 可能尚未收到），或它已交给某台尚未回报的 Computer。成功时也同样依据返回
的状态给出提示。对已经 `cancelled` 的 Task 再次取消是无操作的成功，重复请求无害——包括在前一次取消的锁后等待
的第二次取消。
