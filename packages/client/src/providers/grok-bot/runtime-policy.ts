...
/**
 * Grok Bot has no OS filesystem sandbox and no approval gate. Product policy must stay unrestricted
 * writes plus network; anything stricter is rejected instead of being silently ignored.
 */
export function grokBotRuntimePolicy(_snapshot: EffectiveRuntimeSnapshot): AgentRuntimePolicy {
  return {
...
export function validateGrokBotRuntimePolicy(snapshot: EffectiveRuntimeSnapshot): InputRejectReason | undefined {
