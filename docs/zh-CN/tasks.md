...
Task 详情列出话题内消息的投递作为执行记录。所有 Task 时间线都按时间正序展示，包括私聊、群聊和频道话题。
每轮先显示入站消息、再显示 Agent 回复，最新轮次位于底部。**加载更早活动**位于对话上方，将更早记录插入
顶部，并保持当前可见消息的阅读位置。API 仍先获取最新一页，Web 将完整已加载记录转成正序展示。
Task 列表排序不变。

两类投递被排除：thread Session 已拥有的消息在
