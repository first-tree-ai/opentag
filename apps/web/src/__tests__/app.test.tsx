...
describe("OpenTag Web App Shell", () => {
  ...
  it("keeps a repair exit in every Computer state", async () => {
    ...
    for (const state of states) {
      ...
      // Every state keeps an exit; only its wording varies with what the state can be exited to.
      expect(
        within(row)
          .getByRole("link", { name: state.exit ?? "View Computer" })
          .getAttribute("href"),
      ).toBe(`/agents/${agentId}/settings/computer`);
      rendered.unmount();
    }
  });
