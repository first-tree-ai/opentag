...
- **CLI。** 登录时记录在 OpenTag home 中、与 `credentials.json` 并列的 `account-identity.json` 里的 Account，这样在
  ...
  时的 server，且只在它指向报告要发往的 server 时才生效；文件损坏只让报告失去 `userId`，不影响其他内容。它还通过一个
  指纹——refresh token 的 SHA-256，绝不是 token 本身——与写入时并列的那份凭据绑定，凭据一旦被替换就会被忽略：回滚是
  受支持的操作序列，较旧的 CLI 以另一个 Account 登录时只重写 `credentials.json`，若没有这层绑定，之前的 Account 就会被
  归到新 token 名下。省略 Account 是安全的方向，错误归因永远不是。token 刷新会轮换 refresh token，因此刷新路径会把指纹
  一并带过去，但只针对能证明属于刚被替换的那份凭据的身份文件；过期的文件原样保留，并继续被忽略。**在该文件出现
