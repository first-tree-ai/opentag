 ...
 *
 * Feishu and Lark are two separate channels, not one product under two names, and what OpenTag
 * supports today is Feishu. A Lark channel would arrive as its own provider beside `feishu` and
 * `slack` rather than as a second name for this one, so no label here carries an apposition.
 *
