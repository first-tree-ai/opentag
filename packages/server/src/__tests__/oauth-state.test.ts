...
describe("OAuthFlowService", () => {
  it("binds state to a signed context without putting an invitation bearer in state", async () => {
    ...
    const started = await service.start(`/invite/${token}`);
    expect(JSON.stringify(decodeJwt(started.state))).not.toContain(token);
    expect(await service.verify(started.state, started.context)).toMatchObject({ next: `/invite/${token}` });
    expect(invitationTokenFromNext(`/invite/${token}`)).toBe(token);
  });
  ...
  it("rejects flow substitution, expiry, and unsafe redirect destinations", async () => {
    ...
    const first = await service.start("/admin/teams/example");
    const second = await service.start("/admin");
    ...
    expect(() => validateOAuthNext("/admin\\evil")).toThrow();
  });
});
