...
async function collectOfflineEvents(options: RunnerAcceptanceOptions): Promise<{
  ...
  // version expectations whenever the image identity is known. Generic tools keep the 10s
  // default; only the native `pi --version` provider probe gets the same bounded startup
  // allowance as `createTrackedFactory`.
