...
| `grantedCapabilities` | Keep on the installation | Actual token scopes reported by Slack. Routes sync a sanitized snapshot. Active/readiness projections require all seven fixed scopes. |
...
- The same seven bot scopes and subscribed bot events as the checked-in Public Distribution manifest, including
