...
const INVOCATIONS: Record<string, readonly unknown[]> = {
  ...
  skills: [{}],
  skill: ["release-notes"],
  skillMarkdown: ["release-notes"],
  uploadSkill: [new Blob(["zip"], { type: "application/zip" }), { onConflict: "replace" }],
  deleteSkill: ["release-notes"],
  skillAgents: ["release-notes"],
  agentSkills: [ID],
  replaceAgentSkills: [ID, ["release-notes"]],
