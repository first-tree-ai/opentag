...
describe("MCP daily use", () => {
  ...
  it("searches descriptions, enters detail and restores search and focus on return", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /create_issue/ }));
    expect(screen.getByText("Description provided by linear")).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Input schema" }));
    expect(await screen.findByText(/"type": "object"/)).toBeTruthy();
    ...
    expect(document.activeElement?.textContent).toContain("create_issue");
  });
