...
Team Admin 可在 Agent 的 **Runtime** 页面或对应的 `agent update` 参数中管理 model、reasoning effort 和单个 Turn
的最长执行时间。model 或 reasoning 留空表示交由 Provider 管理；duration 留空则使用 OpenTag 的 30 分钟默认值。
显式填写的 Provider-native 值会在绑定 Computer 准备 Runtime 时校验，OpenTag 不会静默替换。
